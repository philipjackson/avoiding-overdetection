...
cluster = [randi(size(population.template,1),1,population.clust);...
           randi(size(population.template,2),1,population.clust)];
...
for ind = 1:population.N
	...
	while status == 0
		...
		if rand < population.clustprob
			...
			Y = randi(size(population.template,1));
			X = randi(size(population.template,2));
		end
